...
const EdgeDialog = ({
  ...
  // Update a property key or value
  ...
  const handleSubmit = (e) => {
    ...
    // Build a properties object from the array
    ...
    onSave(edgeData, isEditing); // <<< Pass isEditing flag
  };
              ...
              <label style={dialogStyles.label}>Properties:</label>
               {properties.length > 0 && properties.map((prop, index) => (
                  <div key={index} style={dialogStyles.propertyRow}>
                     <input type="text" value={prop.key} onChange={(e) => handlePropertyChange(index, 'key', e.target.value)} placeholder="Property name" style={dialogStyles.propertyInput}/>
                     <input type="text" value={prop.value} onChange={(e) => handlePropertyChange(index, 'value', e.target.value)} placeholder="Value" style={dialogStyles.propertyInput} />
                     <button type="button" onClick={() => handleRemoveProperty(index)} style={{...dialogStyles.button, ...dialogStyles.removeButton}}>-</button>
                  </div>
               ))}
